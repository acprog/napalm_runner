...
static int floater_interpolate(bitrate_manager_state *bm,vorbis_info *vi,
  ...
  if(bin+1<bm->queue_bins){
    ...
    if(my_fabs(hibitrate-desired_rate) < my_fabs(lobitrate-desired_rate))bin++;
  }
  return(bin);
}
...
void vorbis_bitrate_init(vorbis_info *vi,bitrate_manager_state *bm){
  ...
  if(bi){
	...
	bi->queue_hardmin>0)){
      ...
      bm->queue_bins=bins;
      bm->queue_binned=(ogg_uint32_t*)_ogg_calloc(maxpackets,bins*sizeof(*bm->queue_binned));
      bm->queue_actual=(ogg_uint32_t*)_ogg_calloc(maxpackets,sizeof(*bm->queue_actual));
      
      if((bi->queue_avgmin>0 || bi->queue_avgmax>0) &&
	 bi->queue_avg_time>0){
	
	bm->avg_binacc=(long*)_ogg_calloc(bins,sizeof(*bm->avg_binacc));
	...
	bm->minmax_binstack=(long*)_ogg_calloc((bins*2+1)*bins*2,
					sizeof(*bm->minmax_binstack));
	bm->minmax_posstack=(long*)_ogg_calloc((bins*2+1),
				      sizeof(*bm->minmax_posstack));
	bm->minmax_limitstack=(long*)_ogg_calloc((bins*2+1),
	...
	bm->minmax_tail= -1;
      ...
      bm->packetbuffers=(oggpack_buffer*)_ogg_calloc(maxpackets,sizeof(*bm->packetbuffers));
      bm->packets=(ogg_packet*)_ogg_calloc(maxpackets,sizeof(*bm->packets));
      for(i=0;i<maxpackets;i++)
	oggpack_writeinit(bm->packetbuffers+i);
      
    }else{
      bm->packetbuffers=(oggpack_buffer*)_ogg_calloc(1,sizeof(*bm->packetbuffers));
      bm->packets=(ogg_packet*)_ogg_calloc(1,sizeof(*bm->packets));
...
int vorbis_bitrate_addblock(vorbis_block *vb){
  ...
  if(!bm->queue_binned){
    ...
    my_memcpy(&temp,bm->packetbuffers,sizeof(vb->opb));
    my_memcpy(bm->packetbuffers,&vb->opb,sizeof(vb->opb));
    my_memcpy(&vb->opb,&temp,sizeof(vb->opb));
  ...
  /* swap packet buffers */
  my_memcpy(&temp,bm->packetbuffers+head,sizeof(vb->opb));
  my_memcpy(bm->packetbuffers+head,&vb->opb,sizeof(vb->opb));
  my_memcpy(&vb->opb,&temp,sizeof(vb->opb));
  ...
  if(bm->minmax_binstack){
	...
	my_memset(&bm->minmax_binstack[stackctr*bins*2],
	       0,
	...
	my_memmove(bm->minmax_binstack,bm->minmax_binstack+bins*2,
		sizeof(*bm->minmax_binstack)*bins*2*bm->minmax_stackptr);
	my_memmove(bm->minmax_posstack,bm->minmax_posstack+1,
		sizeof(*bm->minmax_posstack)*bm->minmax_stackptr);
	my_memmove(bm->minmax_limitstack,bm->minmax_limitstack+1,
...
int vorbis_bitrate_flushpacket(vorbis_dsp_state *vd,ogg_packet *op){
  ...
  if(bm->queue_size==0){
    ...
    my_memcpy(op,bm->packets,sizeof(*op));
    bm->queue_head=0;
...

    {
      ...
      my_memcpy(op,bm->packets+bm->next_to_flush,sizeof(*op));
