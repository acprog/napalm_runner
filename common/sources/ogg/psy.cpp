...
static float ***setup_tone_curves(float curveatt_dB[P_BANDS],float binHz,int n,
  ...
  float *brute_buffer=(float*)(void*)my_alloca(n*sizeof(*brute_buffer));

  float ***ret=(float***)_ogg_malloc(sizeof(*ret)*P_BANDS);

  my_memset(workc,0,sizeof(workc));
  ...
  for(i=0;i<P_BANDS;i++){
    ...
    for(j=0;j<6;j++)
      my_memcpy(workc[i][j+2],tonemasks[i][j],EHMER_MAX*sizeof(*tonemasks[i][j]));
    my_memcpy(workc[i][0],tonemasks[i][0],EHMER_MAX*sizeof(*tonemasks[i][0]));
    my_memcpy(workc[i][1],tonemasks[i][0],EHMER_MAX*sizeof(*tonemasks[i][0]));
    
    /* apply centered curve boost/decay */
    for(j=0;j<P_LEVELS;j++){
      for(k=0;k<EHMER_MAX;k++){
	float adj=center_boost+my_abs(EHMER_OFFSET-k)*center_decay_rate;
    ...
    for(j=0;j<P_LEVELS;j++){
      ...
      my_memcpy(athc[j],ath,EHMER_MAX*sizeof(**athc));
  ...
  for(i=0;i<P_BANDS;i++){
    ...
    bin=my_floor(fromOC(i*.5)/binHz);
    lo_curve=  my_ceil(toOC(bin*binHz+1)*2);
    hi_curve=  my_floor(toOC((bin+1)*binHz)*2);
...
void _vp_psy_init(vorbis_look_psy *p,vorbis_info_psy *vi,
  ...
  p->shiftoc=rint(my_log(gi->eighth_octave_lines*8.f)/my_log(2.f))-1;
...
static void seed_chase(float *seeds, int linesper, long n){
  long  *posstack=(long*)(void*)my_alloca(n*sizeof(*posstack));
  float *ampstack=(float*)(void*)my_alloca(n*sizeof(*ampstack));
  long   stack=0;
  long   pos=0;
  long   i;
...
static void bark_noise_hybridmp(int n,const long *b,
  ...
  float *N=(float*)(void*)my_alloca(n*sizeof(*N));
  float *X=(float*)(void*)my_alloca(n*sizeof(*N));
  float *XX=(float*)(void*)my_alloca(n*sizeof(*N));
  float *Y=(float*)(void*)my_alloca(n*sizeof(*N));
  float *XY=(float*)(void*)my_alloca(n*sizeof(*N));
...
void _vp_noisemask(vorbis_look_psy *p,
  ...
  int i,n=p->n;
  float *work=(float*)(void*)my_alloca(n*sizeof(*work));
...
void _vp_tonemask(vorbis_look_psy *p,
  ...
  float *seed=(float*)(void*)my_alloca(sizeof(*seed)*p->total_octave_lines);
...
static void couple_lossless(float A, float B, 
			    float *qA, float *qB){
  int test1=my_fabs(*qA)>my_fabs(*qB);
  test1-= my_fabs(*qA)<my_fabs(*qB);
  
  if(!test1)test1=((my_fabs(A)>my_fabs(B))<<1)-1;
  if(test1==1){
...
static void precomputed_couple_point(float premag,
  ...
  int offset=31-my_abs(floorA-floorB);
...
static float dipole_hypot(float a, float b){
  if(a>0.){
    if(b>0.)return my_sqrt(a*a+b*b);
    if(a>-b)return my_sqrt(a*a-b*b);
    return -my_sqrt(b*b-a*a);
  }
  if(b<0.)return -my_sqrt(a*a+b*b);
  if(-a>b)return -my_sqrt(a*a-b*b);
  return my_sqrt(b*b-a*a);
...
static int apsort(const void *a, const void *b){
  float f1=my_fabs(**(float**)a);
  float f2=my_fabs(**(float**)b);
  return (f1<f2)-(f1>f2);
}
...
int **_vp_quantize_couple_sort(vorbis_block *vb,
  ...
  if(p->vi->normal_point_p){
    ...
    float **work=(float**)(void*)my_alloca(sizeof(*work)*partition);
	...
	my_qsort(work,partition,sizeof(*work),apsort);
  ...
  for(i=0;i<vi->coupling_steps;i++){
	...
	for(k=0;k<partition;k++){
	  ...
	  if(l<sliding_lowpass){
	    if((l>=limit && my_fabs(rM[l])<postpoint && my_fabs(rA[l])<postpoint) ||
	       (my_fabs(rM[l])<prepoint && my_fabs(rA[l])<prepoint)){
