...
static void _preextrapolate_helper(vorbis_dsp_state *v){
  int i;
  int order=32;
  float *lpc=(float*)(void*)my_alloca(order*sizeof(*lpc));
  float *work=(float*)(void*)my_alloca(v->pcm_current*sizeof(*work));
  long j;
  v->preextrapolate=1;
...
int vorbis_analysis_blockout(vorbis_dsp_state *v,vorbis_block *vb){
  ...
  for(i=0;i<vi->channels;i++){
    ...
    my_memcpy(vbi->pcmdelay[i],v->pcm[i],(vb->pcmend+beginW)*sizeof(*vbi->pcmdelay[i]));
  ...
  /* advance storage vectors and clean up */
  {
    ...
    if(movementW>0){
      ...
      for(i=0;i<vi->channels;i++)
	my_memmove(v->pcm[i],v->pcm[i]+movementW,
