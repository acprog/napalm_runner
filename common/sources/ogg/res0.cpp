...
vorbis_look_residue *res0_look(vorbis_dsp_state *vd,
  ...
  look->partvals=rint(my_pow((float)look->parts,(float)dim));
...
static long **_01class(vorbis_block *vb,vorbis_look_residue *vl,
  ...
  for(i=0;i<ch;i++){
    ...
    my_memset(partword[i],0,n/samples_per_partition*sizeof(*partword[i]));
  }
  ...
  for(i=0;i<partvals;i++){
	...
	if(my_fabs(in[j][offset+k])>max)max=my_fabs(in[j][offset+k]);
	ent+=my_fabs(rint(in[j][offset+k]));
      }
      ent*=scale;
...
static long **_2class(vorbis_block *vb,vorbis_look_residue *vl,float **in,
  ...
  my_memset(partword[0],0,n*ch/samples_per_partition*sizeof(*partword[0]));

  for(i=0,l=info->begin/ch;i<partvals;i++){
    float magmax=0.f;
    float angmax=0.f;
    for(j=0;j<samples_per_partition;j+=ch){
      if(my_fabs(in[0][l])>magmax)magmax=my_fabs(in[0][l]);
      for(k=1;k<ch;k++)
	if(my_fabs(in[k][l])>angmax)angmax=my_fabs(in[k][l]);
      l++;
    }
...
static int _01forward(vorbis_block *vb,vorbis_look_residue *vl,
  ...
  my_memset(resbits,0,sizeof(resbits));
  my_memset(resvals,0,sizeof(resvals));
...
static int _01inverse(vorbis_block *vb,vorbis_look_residue *vl,
  ...
  int ***partword=(int***)(void*)my_malloc(ch*sizeof(*partword));
