...
void _ve_envelope_init(envelope_lookup *e,vorbis_info *vi){
  ...
  for(i=0;i<n;i++){
    e->mdct_win[i]=my_sin(i/(n-1.)*M_PI);
  ...
  for(j=0;j<VE_BANDS;j++){
    ...
    for(i=0;i<n;i++){
      e->band[j].window[i]=my_sin((i+.5)/n*M_PI);
...
static int _ve_amp(envelope_lookup *ve,
  ...
  float *vec=(float*)(void*)my_alloca(n*sizeof(*vec));
...
void _ve_envelope_shift(envelope_lookup *e,long shift){
  ...
  my_memmove(e->mark,e->mark+smallshift,(smallsize-smallshift)*sizeof(*e->mark));
