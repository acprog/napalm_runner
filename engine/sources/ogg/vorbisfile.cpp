...
static int _ov_open1(void *f,OggVorbis_File *vf,char *initial,
  ...
  if(initial){
    ...
    my_memcpy(buffer,initial,ibytes);
...
static void _ov_getlap(OggVorbis_File *vf,vorbis_info *vi,vorbis_dsp_state *vd,
  ...
  while(lapcount<lapsize){
	...
	my_memcpy(lappcm[i]+lapcount,pcm[i],sizeof(**pcm)*samples);
      lapcount+=samples;
  ...
  if(lapcount<lapsize){
    ...
    if(samples==0){
      for(i=0;i<vi->channels;i++)
	my_memset(lappcm[i]+lapcount,0,sizeof(**pcm)*lapsize-lapcount);
      lapcount=lapsize;
    }else{
      if(samples>lapsize-lapcount)samples=lapsize-lapcount;
      for(i=0;i<vi->channels;i++)
...
int ov_crosslap(OggVorbis_File *vf1, OggVorbis_File *vf2){
  ...
  lappcm=(float**)(void*)my_alloca(sizeof(*lappcm)*vi1->channels);
  ...
  for(i=0;i<vi1->channels;i++)
    lappcm[i]=(float*)(void*)my_alloca(sizeof(**lappcm)*n1);
...
static int _ov_64_seek_lap(OggVorbis_File *vf,ogg_int64_t pos,
  ...
  lappcm=(float**)(void*)my_alloca(sizeof(*lappcm)*ch1);
  for(i=0;i<ch1;i++)
    lappcm[i]=(float*)(void*)my_alloca(sizeof(**lappcm)*n1);
