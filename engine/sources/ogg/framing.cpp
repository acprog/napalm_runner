...
int ogg_stream_packetin(ogg_stream_state *os,ogg_packet *op){
  ...
  if(os->body_returned){
    ...
    if(os->body_fill)
      my_memmove(os->body_data,os->body_data+os->body_returned,
	      os->body_fill);
    os->body_returned=0;
  }
  ...
  my_memcpy(os->body_data+os->body_fill,op->packet,op->bytes);
...
int ogg_stream_flush(ogg_stream_state *os,ogg_page *og){
  ...
  my_memcpy((void*)os->header,(void*)"OggS",4);
  ...
  os->lacing_fill-=vals;
  my_memmove(os->lacing_vals,os->lacing_vals+vals,os->lacing_fill*sizeof(*os->lacing_vals));
  my_memmove(os->granule_vals,os->granule_vals+vals,os->lacing_fill*sizeof(*os->granule_vals));
...
char *ogg_sync_buffer(ogg_sync_state *oy, long size){
  ...
  if(oy->returned){
    oy->fill-=oy->returned;
    if(oy->fill>0)
      my_memmove(oy->data,oy->data+oy->returned,oy->fill);
    oy->returned=0;
  }
...
long ogg_sync_pageseek(ogg_sync_state *oy,ogg_page *og){
  ...
  if(oy->headerbytes==0){
    ...
    if(my_memcmp((char*)page,"OggS",4))goto sync_fail;
  ...
  /* The whole test page is buffered.  Verify the checksum */
  {
    ...
    my_memcpy(chksum,page+22,4);
    my_memset(page+22,0,4);
    ...
    /* Compare */
    if(my_memcmp(chksum,(char*)page+22,4)){
      /* D'oh.  Mismatch! Corrupt page (or miscapture and not a page
	 at all) */
      /* replace the computed checksum with the one actually read in */
      my_memcpy(page+22,chksum,4);
 ...
 sync_fail:
  ...
  next=(unsigned char*)my_memchr(page+1,'O',bytes-1);
  if(!next)
...
int ogg_stream_pagein(ogg_stream_state *os, ogg_page *og){
  ...
  /* clean up 'returned data' */
  {
    ...
    if(br){
      os->body_fill-=br;
      if(os->body_fill)
	my_memmove(os->body_data,os->body_data+br,os->body_fill);
      os->body_returned=0;
    }
	...
	my_memmove(os->lacing_vals,os->lacing_vals+lr,
		(os->lacing_fill-lr)*sizeof(*os->lacing_vals));
	my_memmove(os->granule_vals,os->granule_vals+lr,
  ...
  if(bodysize){
    ...
    my_memcpy(os->body_data+os->body_fill,body,bodysize);
