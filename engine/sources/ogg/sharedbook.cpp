...
long _float32_pack(float val)
{
  ...
  exp= my_floor(my_log(val)/my_log(2.f));
...
float _float32_unpack(long val)
{
  ...
  if(sign)mant= -mant;
  return(my_ldexp(mant,exp-(VQ_FMAN-1)-VQ_FEXP_BIAS));
}
...
ogg_uint32_t *_make_words(long *l,long n,long sparsecount){
  ...
  my_memset(marker,0,sizeof(marker));
...
long _book_maptype1_quantvals(const static_codebook *b){
  long vals=my_floor(my_pow((float)b->entries,1.f/b->dim));
  ...
  if(b->maptype==1 || b->maptype==2){
	...
	if((sparsemap && b->lengthlist[j]) || !sparsemap){
	  ...
	  for(k=0;k<b->dim;k++){
	    ...
	    val=my_fabs(val)*delta+mindel+last;
...
void vorbis_staticbook_clear(static_codebook *b){
  if(b->allocedp){
    ...
    if(b->nearest_tree){
      ...
      my_memset(b->nearest_tree,0,sizeof(*b->nearest_tree));
      _ogg_free(b->nearest_tree);
    }
    if(b->thresh_tree){
      _ogg_free(b->thresh_tree->quantthresh);
      _ogg_free(b->thresh_tree->quantmap);
      my_memset(b->thresh_tree,0,sizeof(*b->thresh_tree));
...
int vorbis_book_init_decode(codebook *c,const static_codebook *s){
  ...
  /* two different remappings go on here.  
    ...
    ogg_uint32_t **codep=(ogg_uint32_t**)my_malloc(sizeof(*codep)*n);
    ...
    my_qsort(codep,n,sizeof(*codep),sort32a);

    sortindex=(int*)my_malloc(n*sizeof(*sortindex));
